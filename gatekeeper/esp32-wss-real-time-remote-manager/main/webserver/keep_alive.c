...
typedef enum {
    ...
    CLIENT_ACTIVE,
    STOP_TASK,
} __attribute__((__packed__)) Client_Fd_Action_Type_t;

typedef struct {
    Client_Fd_Action_Type_t type;
    int fd;
    uint64_t last_seen;
} __attribute__((__packed__)) Client_Fd_Action_t;
...
} __attribute__((__packed__)) Wss_Keep_Alive_Storage_t;

typedef struct wss_keep_alive_storage* wss_keep_alive_t;

static const char *TAG = "SRSw-keep_alive";
